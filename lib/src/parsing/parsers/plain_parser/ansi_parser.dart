...
final class AnsiParser {
  ...
  AnsiMatches<AnsiPlainState>? _matches;

  AnsiParser(this.input);

  @visibleForTesting
  bool get isParsed => _matches?._parsingResult != null;

  AnsiMatches<AnsiPlainState> get matches => _matches ?? _AnsiMatchesV1(input);

  AnsiPlainState get endState => matches._requireParsingResult.endState;
  ...
  AnsiPlainState stateAtPos(int pos) {
    var end = 0;
  ...
  String substring(
      ...
      switch (entity) {
        case AnsiPlainText():
          ...
          if (pos >= start) {
            if (buf.isEmpty) {
              buf.write(
                AnsiPlainState.defaults.transitTo(m.state),
              );
            }
            buf.write(
              string.substring(
    ...
    if (close && currentState != null) {
      buf.write(currentState.transitTo(AnsiPlainState.defaults));
    }
