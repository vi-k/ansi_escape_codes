...
только читает escape-коды, ни к чему 900 констант для их записи. Стили и
парсер живут в одной библиотеке, поэтому `style.dart` приносит оба: написать
стиль и прочитать его обратно — две половины одной поверхности.
...
остальное — работа со строками, идущая везде, где идёт Dart, включая веб и
WebAssembly. Втянуть их через зонтичный вход значило бы сделать весь пакет
...
`Styles.bold`, `Styles.italic` — и 256-цветная таблица трижды:
...
`SGR 0` не очистит непрозрачное оформление. Private CSI и CSI с
intermediate-байтами копируются на своём месте, но replayable SGR-состоянием не
считаются.
...
и его просят по имени: это единственная точка входа, которая доходит до
`dart:io`:
...
ширины, в которую их укладывать, нет. Расстояние, уходящее за ширину, позиции
