...
смог закончить, — незавершённого `OSC`, голого `ESC`, `CSI` без финального
байта. Всё написанное среди её байтов читается как её часть, поэтому текст
встаёт перед последовательностью, а хвост копируется как пришёл:
...
того, что здесь не имеет смысла, и `CsiPrivate` — для последовательностей
