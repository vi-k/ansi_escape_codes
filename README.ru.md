...
Ни одна из вставок не попадает внутрь последовательности, которую парсер не
смог закончить, — управляющей строки без терминатора, будь то `OSC`, `DCS`,
`SOS`, `PM` или `APC`; голого `ESC`; `CSI` без финального байта. Всё
написанное среди её байтов читается как её часть, поэтому текст встаёт перед
последовательностью, а хвост копируется как пришёл:

```dart
print(Parser('aa\x1B]0;title').insertAfter(2, 'X')); // 'aaX\x1B]0;title'
```

Один шов остался непокрытым: там, где управляющую строку оборвал неоконченный
код — голый `ESC`, `CSI` без финального байта, — `insertAfter` по-прежнему
встаёт среди байтов строки, как вставал с тех пор, когда `OSC` был единственной
строкой, с которой это могло случиться; `insertBefore` встаёт там перед
строкой, как и везде.

`CSI` без финального байта — единственный, за которым могут стоять параметры,
и парсер отдаёт их текстом, хотя терминал читает их как часть
последовательности. У позиции среди них верного ответа нет — перед
последовательностью это раньше символов, которые для вызывающего идут до неё,
а на запрошенном месте это внутри последовательности, — поэтому она
отклоняется:
...
последовательность. Позиция вне плоского текста — по-прежнему `RangeError`,
как и везде.
