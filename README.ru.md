...
код — голый `ESC`, `CSI` без финального байта, `ESC` на промежуточном байте, —
`insertAfter` по-прежнему встаёт среди байтов строки, как вставал с тех пор,
когда `OSC` был единственной строкой, с которой это могло случиться;
`insertBefore` встаёт там перед строкой, как и везде.
