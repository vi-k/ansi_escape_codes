...
Поскольку `bold` и `dim` нельзя задать одновременно, в ANSI для сброса обоих
служит одна последовательность: `resetBoldAndDim`.
...
| `style.dart` | 81 | стили, состояние и парсер с таблицами управляющих функций — без таблиц готовых строк |
| `extensions.dart` | 7 | расширения `String` и два перечисления, названные в их сигнатурах |
