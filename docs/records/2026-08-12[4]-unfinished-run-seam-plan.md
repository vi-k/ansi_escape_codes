...
> **Состояние документа**
>
> - **Тип:** план реализации, 2026-08-12, база `fix/unfinished-run-seam`
>   @ `5898e10`
> - **Статус:** исполнен, влит в `main` мержем `41068fd`
> - **Актуальность:** реализует
>   `docs/records/2026-08-12[3]-unfinished-run-seam-design.md`; спека
>   старше плана, при расхождении права спека
> - **Чему не верить:** номера строк поедут. И: **ожидания в планах этого
>   репозитория оказывались неверны десять раз за три волны**, каждый раз
>   их ловил исполнитель, снимавший значения пробником. Читать как
>   намерение, а не как описание поведения
> - **Поправка от 2026-08-12.** Общие ограничения запрещают трогать
>   условие отказа; запрет снят владельцем по итогам задания 2 — см.
>   шапку `2026-08-12[3]`. Добавлено **задание 5**: довести отказ до
>   симметрии, чтобы `insertAfter` отклонял там же, где отклоняет
>   `insertBefore`. Его бриф лежит в рабочем каталоге волны, не здесь
> - **Ещё поправка.** Код задания 2 в этом плане **недостаточен**:
>   `substring` ведёт обход руками и поле не заполняло, а сброс на входе
>   в `nextPiece()` ронял череду при повторном вопросе к исчерпанному
>   обходу. Исполнитель свёл правило в один `_Walk.takeCode`, который
>   зовут оба обхода. Верна реализация, а не текст плана
