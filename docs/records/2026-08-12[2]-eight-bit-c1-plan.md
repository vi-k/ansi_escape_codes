...
> **Состояние на 2026-08-16:** исполнен и влит в `main` мержем `f6061a8`.
> Читать как намерение, а не как описание поведения: номера строк даны по
> базе `0ef28d0` и поехали, а **четыре ожидания оказались неверны** —
> мутация в задании 1 (расширение регулярки парсеру не видно, он ищет
> `ESC` сканером), «третий тест падает до правки» в задании 2 (`U+00A0`
> управляющим не был никогда), «устаревших дартдоков один» там же (их
> три), «половинчатая правка пройдёт весь набор зелёной» в задании 4
> (верно только для половины «сканер»); все четыре поймали исполнители,
> снимавшие значения пробником, ни один не подогнал тест под код.
> **Что это:** план реализации: 8-битные C1 становятся видимы показу и
> удалению управляющих кодов, разбор escape-последовательностей не
> меняется; спека старше плана, при расхождении права спека.
> **Связанные записи:** `2026-08-12[1]-eight-bit-c1-design.md`.
