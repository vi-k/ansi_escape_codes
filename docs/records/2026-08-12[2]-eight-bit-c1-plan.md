...
>   `0ef28d0`
> - **Статус:** исполнен, влит в `main` мержем `f6061a8`
> - **Актуальность:** реализует
>   `docs/records/2026-08-12[1]-eight-bit-c1-design.md`; спека старше
>   плана, при расхождении права спека
> - **Чему не верить:** номера строк даны по базе и поехали. И, важнее:
>   **четыре ожидания этого плана оказались неверны** — мутация в
>   задании 1 (расширение регулярки парсеру не видно, он ищет `ESC`
>   сканером), «третий тест падает до правки» в задании 2 (`U+00A0`
>   управляющим не был никогда), «устаревших дартдоков один» там же (их
>   три), и «половинчатая правка пройдёт весь набор зелёной» в задании 4
...
>   Читать этот план как намерение, а не как описание поведения
