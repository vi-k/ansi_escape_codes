...
> **Состояние документа**
>
> - **Тип:** план по принятой спеке `2026-08-14[1]`
> - **Статус:** принят к исполнению владельцем 2026-08-14
> - **Актуальность:** написан против ветки
>   `fix/inserted-unfinished-reemission` @ `20121df`
> - **Чему не верить:** числа полных ворот и CI здесь ещё не сняты; их
>   следует записать в новый handoff только после фактических прогонов.
>   Код корпуса в Task 1, Step 3 вычисляет `plain` через проверяемый
>   `Parser` и после саморевью отклонён: реализация теста должна нести
>   пары «вход → ожидаемый plain» литералами; остальной план не меняется
