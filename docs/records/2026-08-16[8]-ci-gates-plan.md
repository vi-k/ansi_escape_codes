> **Состояние на 2026-08-16:** выполнен целиком, девятью коммитами в ветке
> `fix/ci-gates`. Четыре расхождения с текстом ниже, найденные исполнением:
>
> 1. **Шаг проверки YAML не работает:** `pyyaml` в системе нет. Использовать
>    `ruby -ryaml -e 'YAML.load_file("...")'`.
> 2. **`library;` нужен и новым инструментам.** План этого не говорил, а
>    `check_entry_points.dart` и `generate.dart` его имеют (строки 39 и 18):
>    без него `dangling_library_doc_comments` красит `--fatal-infos`.
> 3. **Появился четвёртый оракул `checkReportRoot`,** которого в плане нет.
>    Пробник задания 3 вскрыл, что отчёт из чужого дерева даёт 98 строк
>    диагностики вместо одной; см. коммит `e9f511f`.
> 4. **Групповые комментарии в списке исключений пришлось убрать:**
>    сортировка, которую требует тест, перемешивает группы, и подписи
>    начинают врать.
>
