...
> **Состояние документа**
>
> - **Тип:** план по принятой спеке `2026-08-14[4]`
> - **Статус:** подготовлен 2026-08-14, ждёт решения владельца об исполнении
> - **Актуальность:** написан против ветки `fix/preserve-unmodelled-sgr` @
>   `fd978c0`
> - **Чему не верить:** числа полных ворот, покрытия и CI здесь ещё не
>   сняты; новый handoff получает их только после фактических прогонов.
>   Байты в разделе baseline сняты живым пробником на этом коммите; байты
>   будущего поведения являются требованиями принятой спеки и сначала
>   должны быть увидены красными
> - **Поправка реализации 2026-08-14:** exact-ожидание Task 3 для
>   `CSI 11 m A CSI 10 m B` противоречит принятой спеке, которая разрешает
>   канонизацию известных функций, и существующему контракту
>   `State.transitTo(Style.terminalColors)`. Живой пробник после typed
>   mapping даёт `CSI 11 m A CSI 0 m B` на трёх parser-выходах и тот же
>   хвост после начального `CSI 0 m` у `Printer`; это сохраняет требуемую
>   терминальную семантику и является правильным ожиданием.
