...
void main() {
  group('the index of a colour in the 256-colour table:', () {
    test('is counted off the cube the table keeps at 16', () {
      ...
      expect(rgb256(0, 0, 0), 16, reason: 'the first colour of the cube');
      expect(rgb256(0, 0, 1), 17);
      expect(rgb256(0, 1, 0), 22);
      expect(rgb256(1, 0, 0), 52);
      expect(rgb256(2, 3, 4), 110);
      expect(rgb256(5, 5, 5), 231, reason: 'the last colour of the cube');
    });

    test('takes six levels of each and no more', () {
      expect(() => rgb256(6, 0, 0), throwsA(isA<IndexError>()));
      expect(() => rgb256(0, 6, 0), throwsA(isA<IndexError>()));
      expect(() => rgb256(0, 0, 6), throwsA(isA<IndexError>()));
      expect(() => rgb256(-1, 0, 0), throwsA(isA<IndexError>()));
    });

    test('is counted off 232 for the greys', () {
      expect(gray256(0), 232, reason: 'the darkest grey');
      expect(gray256(23), 255, reason: 'the lightest grey');
    });

    test('takes twenty-four greys and no more', () {
      expect(() => gray256(24), throwsA(isA<IndexError>()));
      expect(() => gray256(-1), throwsA(isA<IndexError>()));
    });
  });
  ...
  group('setting a colour from the 256-colour table:', () {
    ...
    test('and the parser reads it back', () {
      expect(
        Parser(fg256(rgb256(5, 0, 0))).finalState.foregroundColor,
        Color256.rgb(5, 0, 0),
        reason: 'the far red corner of the cube',
      );
      expect(
        Parser(bg256(gray256(0))).finalState.backgroundColor,
        Color256.gray(0),
      );
    });
  });
